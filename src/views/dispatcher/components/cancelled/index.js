                    ...
                    return[
                        <Card key={key} className="text-xs hover:shadow-lg transition duration-150 ease-in-out dark:border dark:border-gray-600 dark:border-solid mb-3" header={`Order ID : ${item.order_id}`} headerExtra={headerExtraContent} headerBorder={true}>
                            ...
                            <div className="flex justify-end mt-3">
                                ...
                                <Button size="sm" variant="solid" color="green" loading={loading.value && loading.status === "accepted" && loading.id === item.order_id? true : false} onClick={(event) => handleUpdate(event, item.order_id, "accepted", item.order_number)}>Accept</Button>
                            </div>
                        </Card>
                    ]
                }
