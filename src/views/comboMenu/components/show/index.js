...
import EditComboMenu from "./EditComboMenu";
import { toast, Notification } from "components/ui";
import { removeComboMenu } from "services/RestaurantApiServices";
...
const ProductList = (props) => {
  ...
  const deleteRow = async (id) => {
    const resp = await removeComboMenu(id);
    if (resp.data) {
  ...
  return (
    <div>
      ...
      {
        <Drawer
          title="Edit Combo Menu"
          isOpen={isOpen}
          onClose={onDrawerClose}
          onRequestClose={onDrawerClose}
          // footer={Footer}
        >
          <EditComboMenu data={editData} id={id} refresh={refresh} setRefresh={setRefresh} />
        </Drawer>
      }
    </div>
  );
