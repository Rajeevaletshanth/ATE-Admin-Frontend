...
import { apiSignUp, shopApiSignUp } from 'services/AuthService'
...
const SignUpForm = props => {
	...
	const onSignUp = async (values, setSubmitting) => {
		const { username, role, password, email, phone_no } = values
		setSubmitting(true)		
		try {
			let profile = await getAvatar();
			const resp = await shopApiSignUp({ name: username,  authority : {"role" : ["restaurant"]}, phone_no, email, password, avatar: profile })
			if (resp.data) {
				setSubmitting(false)
